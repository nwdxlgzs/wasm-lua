export const LUA_STDLIB = {
  ...
  load: { signature: 'load(chunk [, chunkname [, mode [, env]]])', doc: '编译 Lua 源码；安全档只接受文本，可信档还可显式加载 Lua 字节码。' },
  loadfile: { signature: 'loadfile(filename [, mode [, env]])', doc: '通过实例 VFS 读取并编译 Lua 文件；可信档经授权挂载点也可读取宿主文件夹。' },
  dofile: { signature: 'dofile(filename)', doc: '通过实例 VFS 加载并执行 Lua 文件；可信档可读取授权挂载文件。' },
  ...
  'debug.debug': { signature: 'debug.debug()', doc: '进入交互调试模式。仅可信档提供。' },
  'debug.gethook': { signature: 'debug.gethook([thread])', doc: '读取线程的复合调试钩子。仅可信档提供。' },
  'debug.getinfo': { signature: 'debug.getinfo([thread,] f [, what])', doc: '返回函数或栈帧的调试信息。仅可信档提供。' },
  'debug.getlocal': { signature: 'debug.getlocal([thread,] f, local)', doc: '读取局部变量。仅可信档提供。' },
  'debug.getmetatable': { signature: 'debug.getmetatable(value)', doc: '读取对象元表。仅可信档提供。' },
  'debug.getregistry': { signature: 'debug.getregistry()', doc: '返回注册表。仅可信档提供。' },
  'debug.getupvalue': { signature: 'debug.getupvalue(f, up)', doc: '读取函数 upvalue。仅可信档提供。' },
  'debug.getuservalue': { signature: 'debug.getuservalue(u [, n])', doc: '读取完整 userdata 的用户值。仅可信档提供。' },
  'debug.sethook': { signature: 'debug.sethook([thread,] hook, mask [, count])', doc: '设置用户调试钩子；宿主资源控制钩子仍会保留。仅可信档提供。' },
  'debug.setlocal': { signature: 'debug.setlocal([thread,] level, local, value)', doc: '修改局部变量。仅可信档提供。' },
  'debug.setmetatable': { signature: 'debug.setmetatable(value, table)', doc: '设置对象元表。仅可信档提供。' },
  'debug.setupvalue': { signature: 'debug.setupvalue(f, up, value)', doc: '修改函数 upvalue。仅可信档提供。' },
  'debug.setuservalue': { signature: 'debug.setuservalue(udata, value [, n])', doc: '设置完整 userdata 的用户值。仅可信档提供。' },
  'debug.traceback': { signature: 'debug.traceback([thread,] [message [, level]])', doc: '生成调用栈回溯。仅可信档提供。' },
  'debug.upvalueid': { signature: 'debug.upvalueid(f, n)', doc: '返回 upvalue 的唯一标识。仅可信档提供。' },
  'debug.upvaluejoin': { signature: 'debug.upvaluejoin(f1, n1, f2, n2)', doc: '令两个闭包共享 upvalue。仅可信档提供。' },
  'io.close': { signature: 'io.close([file])', doc: '关闭内存 VFS 文件句柄，安全档及可信档均可用。' },
  ...
  'io.open': { signature: 'io.open(filename [, mode])', doc: '打开实例 VFS 文件；可信档显式授权的目录挂载点也可读写。支持 r/w/a、+ 和 b。' },
